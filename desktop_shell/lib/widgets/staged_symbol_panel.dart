...
class StagedSymbolPanel extends StatefulWidget {
  ...
  final Function(Map<String, dynamic> dConfig, Map<String, dynamic> eConfig)
  onAcceptSymmetric;
...
class _StagedSymbolPanelState extends State<StagedSymbolPanel>
  ...
  void dispose() {
    _animCtrl.dispose();
    _beepTimer?.cancel();
    _sharedQtyCtrl.dispose();
    _sharedProfitCtrl.dispose();
    _sharedDropCtrl.dispose();
    _sharedLoopCtrl.dispose();
    _sharedRungsCtrl.dispose();
    _dQtyCtrl.dispose();
  ...
  void _accept() {
    ...
    final dProfit =
        double.tryParse(_effective(_dProfitCtrl, _sharedProfitCtrl)) ?? 0;
    ...
    final eProfit =
        double.tryParse(_effective(_eProfitCtrl, _sharedProfitCtrl)) ?? 0;
    ...
    if (dQty <= 0 ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Valores inválidos. Revisa la configuración.'),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Expanded(
                child: _buildOverrideSection(
                  'Dorothy (Long)',
                  Colors.tealAccent,
                  _showDorothyOverride,
                  () {
                    setState(
                      () => _showDorothyOverride = !_showDorothyOverride,
                    );
                  },
                  _dQtyCtrl,
                  _dProfitCtrl,
              ...
              Expanded(
                child: _buildOverrideSection(
                  'Elphaba (Short)',
                  Colors.purpleAccent,
                  _showElphabaOverride,
                  () {
                    setState(
                      () => _showElphabaOverride = !_showElphabaOverride,
                    );
                  },
                  _eQtyCtrl,
                  _eProfitCtrl,
  ...
  Widget _buildOverrideSection(
    ...
    TextEditingController drop,
    TextEditingController loop,
    TextEditingController rungs,
        ...
        children: [
          InkWell(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                Text(
                  expanded ? 'override activo' : 'usar simétrico',
                  style: TextStyle(
                    fontSize: 8,
          ...
          if (!expanded)
            Padding(
              ...
              child: Wrap(
                spacing: 6,
                children: [_buildField('Tag', tag, width: 100)],
              ),
            ),
          if (expanded) ...[
            const SizedBox(height: 4),
            Text(
              'Dejar vacío = hereda config simétrica',
              style: TextStyle(
                fontSize: 8,
