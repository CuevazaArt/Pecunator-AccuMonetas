...
class ThusneldaService(BaseHubService):
    ...
    def _make_config(self, **kwargs: Any) -> ThusneldaConfig:
        cfg = ThusneldaConfig(
            symbols_csv=str(kwargs.get("symbols_csv", "BTCUSDT,ETHUSDT")),
            loop_interval_sec=int(kwargs.get("loop_interval_sec", 3600)),
            between_symbol_sec=int(kwargs.get("between_symbol_sec", 5)),
            quote_order_qty_modulo=Decimal(str(kwargs.get("quote_order_qty_modulo", "10"))),
            factor_multiplication=Decimal(str(kwargs.get("factor_multiplication", "0.997"))),
            meta_equity_usdt=Decimal(str(kwargs.get("meta_equity_usdt", "100"))),
            ...
            stop_loss_pct=Decimal(str(kwargs.get("stop_loss_pct", "0.20"))),
